...
export class Sqlite3DatabaseService extends BaseService implements DatabaseServiceInterface {
  ...
  private dbDelete(key: string): Promise<boolean> {
    if (this.connect()) {
      return this.database.execSQL(StateModelSql.delete, [key])
        .toPromise()
        .then((item: any) => {
      ...
      return this.handleError('Cannot connect to db');
    }
  }
  ...
  private dbInsert(key: string, value: any): Promise<boolean> {
    ...
    if (this.connect()) {
      return this.database.execSQL(StateModelSql.insert, [key, String(value)])
        .toPromise()
        .then((item: any) => {
          this.consoleLogRecord(0, item);
          return Promise.resolve(true);
  ...
  private dbUpdate(key: string, value: any): Promise<boolean> {
    ...
    if (this.connect()) {
      return this.database.execSQL(StateModelSql.update, [key, String(value)])
        .toPromise()
        .then((item: any) => {
          this.consoleLogRecord(0, item);
          return Promise.resolve(true);
  ...
  private sync(key: string, value: any): Promise<boolean> {
    ...
    if (this.connect()) {
      return this.database.execSQL(StateModelSql.count, [key])
        .toPromise()
        .then((item: any) => {
